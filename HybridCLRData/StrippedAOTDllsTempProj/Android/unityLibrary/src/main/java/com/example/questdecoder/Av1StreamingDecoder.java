...
public class Av1StreamingDecoder implements ImageReader.OnImageAvailableListener {
    ...
    private static final int MAX_HARDWARE_FRAME_QUEUE = 3;
    ...
    public static class HardwareBufferFrame {
        ...
        HardwareBufferFrame(HardwareBuffer hardwareBuffer, int width, int height, int[] header) {
    ...
    private void startDecoder() {
        ...
        // Force CPU-readable YUV for the first frame so we can validate conversion + forward decoder color metadata
        // to native before switching to zero-copy HardwareBuffer frames.
        final boolean hardwareBuffersEnabled = requestedHardwareBuffersEnabled
                && !(unityAutoCalibrationEnabled && !cpuFirstFrameDecoded);
        ...
        final int maxImages = hardwareBuffersEnabled
                ? Math.max(2, MAX_HARDWARE_FRAME_QUEUE)
                : MAX_QUEUE;
        ...
        try {
            ...
            if (inputBufferId >= 0) {
                ...
                if (inputBuffer != null) {
                    ...
                    nalSubmitted++;
                    logDebug("Submitted AV1 OBU frame to decoder - size: " + frameData.length + ", total submitted: " + nalSubmitted);
                }
            } else {
                noInputBufferCount++;
                long now = System.currentTimeMillis();
        ...
        try (Image image = reader.acquireLatestImage()) {
            ...
            if (decoderUsingHardwareBuffers && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                if (hardwareBuffer != null) {
                    ...
                    HardwareBufferFrame frame = new HardwareBufferFrame(hardwareBuffer, currentWidth, currentHeight, header);
...
// Only push color info into native when we're actually producing HardwareBuffer frames.
    ...
    private void printPeriodicStats() {
        ...
        if (now - lastStatsTime > STATS_INTERVAL_MS) {
            ...
            Log.i(TAG, String.format(
                "Stats: Packets=%.1f/s, Bytes=%.1f KB/s, Reassembled=%.1f/s, Submitted=%.1f/s, Decoded=%.1f/s, Enqueued=%.1f/s, Dequeued=%.1f/s, Queue=%d, NoInputBuf=%d",
